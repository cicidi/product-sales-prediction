...
llm = ChatOpenAI(
    ...
    model="gpt-4o",  # Use the most appropriate model (adjust as needed)
    temperature=0
)

# Create the system message with API documentation
system_message = f"""
You are an AI assistant designed to help users interact with the E-commerce Product Prediction API.
You can perform API calls on behalf of the user and provide insights based on the results.

Here's the documentation of available API endpoints:

{api_documentation}

Remember to call the appropriate API endpoint based on user requests.
If you're unsure about the API capabilities, use the get_available_endpoints tool.

Always respond in a helpful, clear manner. If you can't fulfill a request using the available API endpoints,
...
def main():
    print(f"{Fore.GREEN}=== E-commerce API Chatbot ==={Style.RESET_ALL}")
    print(f"{Fore.YELLOW}Type 'exit' to quit{Style.RESET_ALL}")
    print(f"Connected to API server: {SERVER_URL}")
